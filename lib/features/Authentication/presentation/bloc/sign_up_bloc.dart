  ...
  Stream<SignUpState> mapEventToState(SignUpEvent event) async* {
    if (event is SignUpWithEmailAndPassword) {
      ...
      final failureOrUserEntity =
          await signUpUseCase.signUpWithEmailAndPassword(email: event.email, password: event.password);
     if(failureOrUserEntity != null) yield failureOrUserEntity.fold(
          (failure) => Error(message: SERVER_FAILURE_MESSAGE), (userEntity) => Loaded(userEntity: userEntity));
    }
  }
}
